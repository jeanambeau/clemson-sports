angular.module("clemson_sports.controllers", [])
...
.controller("side_menusCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	popover_template += "			</a>";
	popover_template += "			<a  class=\"item dark-ink\" ng-click=\"openWebView('https://clemson.fanaticsshop.info//wp-admin/')\">";
	popover_template += "			{{ 'Administrator' | translate }}";
	popover_template += "			</a>";
	popover_template += "			<a  class=\"item dark-ink\" ng-href=\"#/clemson_sports/faqs\" ng-click=\"popover.hide()\">";
	popover_template += "			{{ 'Tanya Jawab' | translate }}";
	popover_template += "			</a>";
	popover_template += "			<a  class=\"item dark-ink\" ng-href=\"#/clemson_sports/about_us\" ng-click=\"popover.hide()\">";
...
.controller("categoriesCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$scope.fetchURL = "https://clemson.fanaticsshop.info//wp-json/wp/v2/categories?per_page=100";
	// TODO: categoriesCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "https://clemson.fanaticsshop.info//wp-json/wp/v2/categories?per_page=100&callback=JSON_CALLBACK";
...
.controller("newsCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$rootScope.grid128 = parseInt($rootScope.ionWidth / 128) ;
	$rootScope.grid256 = parseInt($rootScope.ionWidth / 256) ;
	$rootScope.last_edit = "table (news)" ;
	$scope.$on("$ionicView.afterEnter", function (){
		var page_id = $state.current.name ;
		$rootScope.page_id = page_id.replace(".","-") ;
	});
	...
	$scope.fetchURL = "https://clemson.fanaticsshop.info/api/get_posts";
	// TODO: newsCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "https://clemson.fanaticsshop.info/api/get_posts?callback=JSON_CALLBACK";
	...
	localforage.getItem("data_newss_" + $scope.hashURL, function(err, get_newss){
		if(get_newss === null){
			data_newss =[];
		}else{
			data_newss = JSON.parse(get_newss);
			$scope.data_newss =JSON.parse( get_newss);
			$scope.newss = [];
			for(lastPush = 0; lastPush < 50; lastPush++) {
				if (angular.isObject(data_newss[lastPush])){
					$scope.newss.push(data_newss[lastPush]);
				};
			}
			$timeout(function() {
				$ionicLoading.hide();
	...
	if(data_newss.length === 0 ){
		$timeout(function() {
			...
			$http.get(url_request,http_header).then(function(response) {
				data_newss = response.data.post;
				$scope.data_newss = response.data.post;
				// TODO: newsCtrl --|---------- set:localforage
				localforage.setItem("data_newss_" + $scope.hashURL, JSON.stringify(data_newss));
				$scope.newss = [];
				for(lastPush = 0; lastPush < 50; lastPush++) {
					if (angular.isObject(data_newss[lastPush])){
						$scope.newss.push(data_newss[lastPush]);
				...
				$timeout(function() {
					...
					$http.jsonp(url_request,http_header).success(function(data){
						data_newss = data.post;
						$scope.data_newss = data.post;
						$ionicLoading.hide();
						// TODO: newsCtrl --|---------- set:localforage
						localforage.setItem("data_newss_" + $scope.hashURL,JSON.stringify(data_newss));
						controller_by_user();
			...
			$timeout(function() {
				$ionicLoading.hide();
				controller_by_user();
				if(angular.isDefined($scope.data_newss.data)){
					if($scope.data_newss.data.status ===401){
						$scope.showAuthentication();
						return false;
					}
				}
...
// TODO: news_singlesCtrl --|-- 
.controller("news_singlesCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	// TODO: news_singlesCtrl --|-- $scope.scrollTop
	$rootScope.scrollTop = function(){
		$timeout(function(){
			$ionicScrollDelegate.$getByHandle("top").scrollTop();
		},100);
	};
	// TODO: news_singlesCtrl --|-- $scope.toggleGroup
	$scope.toggleGroup = function(group) {
		if ($scope.isGroupShown(group)) {
			$scope.shownGroup = null;
		} else {
	...
	// TODO: news_singlesCtrl --|-- $scope.redirect
	// redirect
	$scope.redirect = function($url){
		$window.location.href = $url;
	};
	...
	// TODO: news_singlesCtrl --|-- $scope.showAuthentication
	$scope.showAuthentication  = function(){
		var authPopup = $ionicPopup.show({
			template: ' This page required login',
			title: "Authorization",
	...
	// Retrieving data
	var itemID = $stateParams.post_1;
	// TODO: news_singlesCtrl --|-- $scope.fetchURL
	$scope.fetchURL = "https://clemson.fanaticsshop.info/api/get_posts";
	// TODO: news_singlesCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "https://clemson.fanaticsshop.info/api/get_posts?callback=JSON_CALLBACK";
	// TODO: news_singlesCtrl --|-- $scope.hashURL
	$scope.hashURL = md5.createHash($scope.fetchURL);
	
	var current_item = [];
	localforage.getItem("data_newss_" + $scope.hashURL, function(err, get_datas){
		if(get_datas === null){
			current_item = [];
		}else{
			if(get_datas !== null){
				...
				for (var i = 0; i < datas.length; i++) {
					if((datas[i].post_1 ===  parseInt(itemID)) || (datas[i].post_1 === itemID.toString())) {
	...
	if( current_item.length === 0 ){
		...
		// TODO: news_singlesCtrl --|-- $http.get
		$http.get($scope.fetchURL,http_header).then(function(response) {
			// Get data single
			var datas = response.data;
			// TODO: news_singlesCtrl --|---------- set:localforage
			localforage.setItem("data_newss_"+ $scope.hashURL,JSON.stringify(datas));
			for (var i = 0; i < datas.length; i++) {
				if((datas[i].post_1 ===  parseInt(itemID)) || (datas[i].post_1 === itemID.toString())) {
					current_item = datas[i] ;
				}
		...
		// TODO: news_singlesCtrl --|-- $scope.doRefresh
	$scope.doRefresh = function(){
		// Retrieving data
		var itemID = $stateParams.post_1;
		var current_item = [];
	...
	// TODO: news_singlesCtrl --|-- controller_by_user
	// controller by user 
	function controller_by_user(){
		try {
...
$ionicConfig.backButton.text("");			
			...
			console.log("%cerror: %cPage: `news_singles` and field: `Custom Controller`","color:blue;font-size:18px","color:red;font-size:18px");
			console.dir(e);
		}
	}
	$scope.rating = {};
...
.controller("post_singlesCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$scope.fetchURL = "https://clemson.fanaticsshop.info//wp-json/wp/v2/posts/" + itemID;
	// TODO: post_singlesCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "https://clemson.fanaticsshop.info//wp-json/wp/v2/posts/" + itemID + "?callback=JSON_CALLBACK";
...
.controller("postsCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$scope.fetchURL = "https://clemson.fanaticsshop.info//wp-json/wp/v2/posts/?categories=-1&per_page=15&page=1";
	// TODO: postsCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "https://clemson.fanaticsshop.info//wp-json/wp/v2/posts/?categories=-1&per_page=15&page=1&callback=JSON_CALLBACK";
...
var UriListing = "https://clemson.fanaticsshop.info//wp-json/wp/v2/posts?categories=-1&per_page=15";    
...
.controller("usersCtrl", function($ionicConfig,$scope,$rootScope,$state,$location,$ionicScrollDelegate,$ionicListDelegate,$http,$httpParamSerializer,$stateParams,$timeout,$interval,$ionicLoading,$ionicPopup,$ionicPopover,$ionicActionSheet,$ionicSlideBoxDelegate,$ionicHistory,ionicMaterialInk,ionicMaterialMotion,$window,$ionicModal,base64,md5,$document,$sce,$ionicGesture,$translate,tmhDynamicLocale){
	...
	$scope.fetchURL = "https://clemson.fanaticsshop.info//wp-json/wp/v2/users/";
	// TODO: usersCtrl --|-- $scope.fetchURLp
	$scope.fetchURLp = "https://clemson.fanaticsshop.info//wp-json/wp/v2/users/?callback=JSON_CALLBACK";
